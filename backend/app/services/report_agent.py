...
class ReportAgent:
    ...
    # 最大工具调用次数（每个章节）- reduced from 5 to 4
    MAX_TOOL_CALLS_PER_SECTION = 4
    MAX_TOOL_RESULT_CHARS_PER_SECTION = 2500
    ...
    def __init__(
        ...
        boost_llm: Optional[LLMClient] = None,
        ...
        Args:
            ...
            llm_client: LLM客户端（可选）— used for planning, outlines, interviews
            boost_llm: Boost LLM客户端（可选）— used for section content synthesis (ReACT loop)
        ...
        self.boost_llm = boost_llm or self.llm
    ...
    def _truncate_tool_result_for_prompt(self, result: str) -> str:
        """Trim oversized tool output before it is appended back into the LLM prompt."""
        if len(result) <= self.MAX_TOOL_RESULT_CHARS_PER_SECTION:
            return result
    ...
    def _generate_section_react(
        ...
        # 构建用户prompt - cap previous sections to save tokens
        if previous_sections:
            previous_parts = []
            for sec in previous_sections:
                # Each section capped at 2000 chars (down from 4000)
                truncated = sec[:2000] + "..." if len(sec) > 2000 else sec
        ...
        # ReACT循环 - reduced from 5/3 to 4/2 to save tokens
        tool_calls_count = 0
        ...
        for iteration in range(max_iterations):
            ...
            # Single LLM call — _create_completion already retries 3x on transient errors.
            # No outer retry needed (was causing 4x3=12 call cascade per iteration).
            try:
                response = self.boost_llm.chat(
                    messages=messages,
                    temperature=0.5,
                    max_tokens=3072,
            ...
            if has_tool_calls:
                ...
                messages.append({
                    ...
                    "content": REACT_OBSERVATION_TEMPLATE.format(
                        tool_name=call["name"],
                        result=self._truncate_tool_result_for_prompt(result),
                        tool_calls_count=tool_calls_count,
                        max_tool_calls=self.MAX_TOOL_CALLS_PER_SECTION,
    ...
    def chat(
        ...
        for iteration in range(max_iterations):
            try:
                ...
                logger.warning(f"Chat LLM error: {_e}")
                response = None
        ...
        try:
            ...
            logger.warning(f"Chat final LLM error: {_e}")
            final_response = None

        if final_response is None:
            return {"response": "", "tool_calls": tool_calls_made, "sources": []}
...
class ReportManager:
    ...
    def get_report(cls, report_id: str) -> Optional[Report]:
        ...
        try:
            ...
            logger.error(f"Failed to read report file {path}: {e}")
            return None
