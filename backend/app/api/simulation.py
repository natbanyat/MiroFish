...
def create_simulation():
    ...
    try:
        ...
        # 提前验证图谱是否包含可用于模拟的实体，避免创建后在 Step 2 才失败
        reader = ZepEntityReader()
        ...
        if filtered_preview.filtered_count == 0:
            if filtered_preview.total_count > 0 and not filtered_preview.entity_types:
                error_message = (
                    "Graph nodes were created but none have entity type labels, "
                    "which makes simulation impossible. Please rebuild the graph."
                )
            else:
                error_message = "No usable entities found in graph. Please check the graph build result."
    ...
    try:
            ...
            if is_prepared:
                ...
                return jsonify({
                    ...
                    "data": {
                        ...
                        "status": "ready",
                        "message": "Preparation already complete, skipping regeneration.",
        ...
        return jsonify({
            ...
            "data": {
                ...
                "task_id": task_id,
                "status": "preparing",
                "message": "Prepare task started. Check progress via /api/simulation/prepare/status",
...
def start_simulation():
    ...
    try:
        ...
        if platform not in ['twitter', 'reddit', 'parallel']:
            return jsonify({
                "success": False,
                "error": f"Invalid platform: {platform}. Options: twitter/reddit/parallel"
            }), 400
...
def interview_agent():
    ...
    try:
        ...
        if not SimulationRunner.check_env_alive(simulation_id):
            return jsonify({
                "success": False,
                "error": "Simulation environment is not running or has been closed. Ensure the simulation has completed and is in command-wait mode."
            }), 400
...
@simulation_bp.route('/reopen-env', methods=['POST'])
def reopen_simulation_env():
    """
    Reopen a completed or stopped simulation environment for post-hoc agent interaction.

    Re-launches the OASIS process from saved profiles and config with max_rounds=1,
    so agents load their profiles and the process enters interview/command-wait mode
    immediately after one short round.
    ...
    try:
        ...
        # If environment is already alive, nothing to do
        if SimulationRunner.check_env_alive(simulation_id):
            return jsonify({
                ...
                "data": {
                    "simulation_id": simulation_id,
                    "already_alive": True,
                    "message": "Environment is already running and ready for interactions"
                }
            })
        ...
        if state.status == SimulationStatus.RUNNING:
            return jsonify({
                "success": False,
                "error": "Simulation is currently running. Wait for it to complete before reopening."
            }), 400

        # Determine which platform(s) were originally enabled
        ...
        # Reset state to READY so SimulationRunner accepts the start call
        state.status = SimulationStatus.READY
        manager._save_simulation_state(state)

        # Re-launch with max_rounds=1 so the process enters interview mode quickly
        run_state = SimulationRunner.start_simulation(
            simulation_id=simulation_id,
            platform=platform,
            max_rounds=1,
        ...
        return jsonify({
            ...
            "data": {
                ...
                "message": "Environment is starting. Poll /env-status until env_alive=true."
            }
        })
        ...
        logger.error(f"Reopen simulation env failed: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
